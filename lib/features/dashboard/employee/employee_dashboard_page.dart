...
class _EmployeeDashboardPageState extends State<EmployeeDashboardPage>
  ...
  bool get _hasActiveFilters => _selectedOrderTypeFilter != null || _selectedScheduleFilter != null;
  int get _activeFiltersCount => (_selectedOrderTypeFilter != null ? 1 : 0) + (_selectedScheduleFilter != null ? 1 : 0);
  ...
  Future<void> _loadEmployeeInvoices({bool loadMore = false, bool refresh = false, String? searchQuery}) async {
    ...
    if (loadMore) {
      if (_isLoadingInvoices || _isLoadingMoreInvoices || !_hasMoreInvoices) return;
      if (mounted) {
        setState(() {
    ...
    try {
      final query = _invoicesSearchQuery.trim().isNotEmpty ? _invoicesSearchQuery.trim() : null;
      ...
      if (mounted) {
        setState(() {
          ...
          _employeeInvoices.sort((a, b) => b.fechaEmision.compareTo(a.fechaEmision));
      ...
      if (mounted) {
        setState(() {
          if (!loadMore) {
            _invoicesErrorMessage = 'Error al cargar el historial de servicios: $e';
          }
        });
      }
    } finally {
  ...
  int _compareOrdersByReservation(WashGoOrder a, WashGoOrder b) {
    ...
    final dtA = _parseOrderDateTime(parsedA.dateTime) ?? DateTime.fromMillisecondsSinceEpoch(0);
    final dtB = _parseOrderDateTime(parsedB.dateTime) ?? DateTime.fromMillisecondsSinceEpoch(0);
  ...
  List<WashGoOrder> get _filteredPendingQueue {
    ...
    if (_selectedOrderTypeFilter != null) {
      list = list.where((order) => order.type == _selectedOrderTypeFilter).toList();
    }
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isSelected ? AppColors.primary.withValues(alpha: 0.3) : Colors.grey.shade300,
          ),
        ),
        child: Row(
          mainAxisSize: MainAxisSize.min,
                ...
                Expanded(
                  child: Wrap(
                    ...
                    children: [
                      ...
                      _buildChoiceChip(
                        label: '🚗 A Domicilio',
                        isSelected: _selectedOrderTypeFilter == OrderType.DELIVERY,
                        onTap: () {
                          setState(() {
      ...
      if (currentBusinessId == null) {
        if (_employeeStatus == EmployeeStatus.PENDING) {
          ...
          if (mounted) {
            setState(() {
              _isLoading = false;
              _errorMessage = 'No estás asociado a ninguna lavandería activa. Por favor, asóciate con un código de negocio.';
            });
          }
          return;
        }
  ...
  void _subscribeToOrders() {
    ...
    _ordersSubscription = _orderRepository.watchBusinessOrders(_businessId!).listen(
      (orders) {
      ...
      onError: (error) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Error al sincronizar pedidos en tiempo real: $error'),
  ...
  Future<void> _fetchAvailability() async {
    if (_businessId == null || _employeeId == null || _employeeStatus == EmployeeStatus.PENDING) return;
    setState(() {
  ...
  Future<void> _acceptOrder(WashGoOrder order) async {
    ...
    if (!_isAvailable) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('No puedes aceptar pedidos mientras estés en descanso / almuerzo. Cambia tu estado a Disponible primero.'),
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('¡Pedido aceptado exitosamente! Se ha movido a tu sección de pedidos.'),
  ...
  Future<void> _updateOrderStatus(WashGoOrder order, OrderStatus newStatus) async {
    // Mostrar loader dialog
    showDialog(
      context: context,
      barrierDismissible: false,
  ...
  Future<void> _showCompletionDialog(WashGoOrder order) async {
        ...
        return AlertDialog(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Ej. Vehículo entregado impecable, champú biodegradable premium...',
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              child: const Text('Confirmar y Facturar', style: TextStyle(color: Colors.white)),
            ),
          ],
        );
      },
    ...
    if (confirmed == true && mounted) {
      ...
      try {
        ...
        String? clientEmail;
        if (order.client.telefono != null && order.client.telefono!.isNotEmpty) {
          try {
            final clientInfo = await _orderRepository.findUserByPhone(order.client.telefono!);
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('¡Pedido completado y factura digital generada con éxito!'),
            backgroundColor: Colors.green,
          ),
        );
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error al finalizar el pedido y generar la factura: $e'),
            backgroundColor: AppColors.error,
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: (_currentNavIndex == 0 && _employeeStatus != EmployeeStatus.PENDING)
            ? PreferredSize(
                              ...
                              children: [
                                Positioned(
                                  ...
                                  child: Container(
                                    ...
                                    child: IconButton(
                                      icon: Icon(
                                        _showFiltersPanel ? Icons.filter_list_off_rounded : Icons.filter_list_rounded,
      ...
      body: _buildBody(),
      floatingActionButton: (_currentNavIndex == 0 && _employeeStatus != EmployeeStatus.PENDING)
  ...
  Widget _buildPendingQueueTab() {
        ...
        children: [
          ...
          Center(
            child: Column(
              ...
              children: [
                Icon(
                  _searchQuery.isNotEmpty ? Icons.search_off_rounded : Icons.assignment_turned_in_outlined,
                  size: 72,
                  color: AppColors.onSurfaceVariant.withValues(alpha: 0.4),
                ),
                const SizedBox(height: 16),
                Text(
                  _searchQuery.isNotEmpty ? 'No se encontraron resultados' : '¡No hay pedidos en cola!',
                  style: GoogleFonts.outfit(
                    fontSize: 20,
                    fontWeight: FontWeight.bold,
                    color: AppColors.onBackground,
    ...
    final displayCount = list.length > _pendingLimit ? _pendingLimit : list.length;
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                            ...
                            children: [
                              Icon(
                                orderType == OrderType.DELIVERY
                                    ...
                                    : Icons.store_rounded,
                                size: 14,
                                color: orderType == OrderType.DELIVERY ? Colors.purple : Colors.blue,
                              ),
                              const SizedBox(width: 4),
                              Text(
                ...
                Row(
                  children: [
                    const Icon(Icons.payment_rounded, size: 16, color: AppColors.onSurfaceVariant),
                    const SizedBox(width: 6),
                    Text(
  ...
  Widget _buildMyOrdersTab() {
        ...
        children: [
          ...
          Center(
            child: Column(
              ...
              children: [
                Icon(
                  _searchQuery.isNotEmpty ? Icons.search_off_rounded : Icons.local_car_wash_outlined,
                  size: 72,
                  color: AppColors.onSurfaceVariant.withValues(alpha: 0.4),
                ),
                const SizedBox(height: 16),
                Text(
                  _searchQuery.isNotEmpty ? 'No se encontraron resultados' : 'Aún no tienes pedidos aceptados',
                  style: GoogleFonts.outfit(
                    fontSize: 20,
                    fontWeight: FontWeight.bold,
                    color: AppColors.onBackground,
    ...
    final displayCount = list.length > _activeLimit ? _activeLimit : list.length;
                  ...
                  child: Row(
                    children: [
                      CircleAvatar(
                        ...
                        backgroundImage: order.client.fotoPerfil != null && order.client.fotoPerfil!.isNotEmpty
                            ? CachedNetworkImageProvider(order.client.fotoPerfil!)
                            : null,
                        child: order.client.fotoPerfil == null || order.client.fotoPerfil!.isEmpty
                            ? Text(
                                order.client.nombreCompleto.substring(0, 1).toUpperCase(),
    ...
    if (status == OrderStatus.ACEPTADO) {
      if (type == OrderType.DELIVERY) {
        return ElevatedButton.icon(
          ...
          label: const Text('Iniciar Ruta (En Camino)', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          style: ElevatedButton.styleFrom(
        ...
        return ElevatedButton.icon(
          ...
          label: const Text('Iniciar Servicio / Lavado', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
          style: ElevatedButton.styleFrom(
      ...
      return ElevatedButton.icon(
        ...
        label: const Text('Llegué al Domicilio / Iniciar Servicio', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        style: ElevatedButton.styleFrom(
          backgroundColor: Colors.teal,
          padding: const EdgeInsets.symmetric(vertical: 12),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
      ...
      return ElevatedButton.icon(
        ...
        label: const Text('Completar y Entregar', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
        style: ElevatedButton.styleFrom(
          backgroundColor: Colors.green,
          padding: const EdgeInsets.symmetric(vertical: 12),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
...
class _CreateWalkInOrderDialog extends StatefulWidget {
  ...
  @override
  State<_CreateWalkInOrderDialog> createState() => _CreateWalkInOrderDialogState();
}
...
class _CreateWalkInOrderDialogState extends State<_CreateWalkInOrderDialog> {
  ...
  Future<void> _loadServices() async {
    try {
      final services = await widget.businessRepository.getBusinessServices(widget.businessId);
      if (mounted) {
        setState(() {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          DropdownButtonFormField<String>(
                            ...
                            items: const [
                              DropdownMenuItem<String>(value: 'Pequeño', child: Text('Pequeño')),
                              DropdownMenuItem<String>(value: 'Mediano', child: Text('Mediano')),
                              DropdownMenuItem<String>(value: 'Grande', child: Text('Grande')),
                          ...
                          DropdownButtonFormField<WashGoService>(
                            ...
                            items: _services.map((service) {
                              return DropdownMenuItem<WashGoService>(
                                value: service,
                                child: Text('${service.nombre} (\$${service.getPriceForCategory(_selectedCategory).toStringAsFixed(2)})'),
                              );
                            }).toList(),
                            onChanged: (val) {
                              setState(() {
