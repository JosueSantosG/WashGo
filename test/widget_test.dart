...
void main() {
  ...
  testWidgets('App builds, shows Splash Screen, and then routes to Home Page', (WidgetTester tester) async {
    ...
    // Verify that the home page is not yet present
    expect(find.text('Buscar lavanderías...'), findsNothing);

    // Let the splash screen animation complete and trigger navigation (2.5 seconds duration)
    await tester.pump(const Duration(milliseconds: 2600));
    await tester.pumpAndSettle();

    // Verify that it has navigated to the Home Page
    expect(find.text('Buscar lavanderías...'), findsOneWidget);
  });
}
